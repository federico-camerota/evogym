...
from utils.features import get_width, get_num_voxels_type, get_elongation, get_eccentricity, get_height, get_size
...
def lazy_speciate(species, structures, shape_features, threshold=2):
    idx = 0
...
def run_se(experiment_name, structure_shape, pop_size, max_evaluations, train_iters, num_cores):
    ...
    while True:
        ...
        structures = sorted(structures, key=lambda struct: struct.fitness, reverse=True)
        ...
        for structure in structures:
            out += "\t\t".join([str(structure.label), str(structure.fitness), str(get_size(structure.body)),
                                ...
                                str(get_num_voxels_type(structure.body, 1)),
                                str(get_num_voxels_type(structure.body, 2)),
                                str(get_num_voxels_type(structure.body, 3)),
                                str(get_num_voxels_type(structure.body, 4)),
